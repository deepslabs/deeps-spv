...
fn sgx_registration(config: &config::ConfigCli) {
    if config.sgx_enable {
        ...
        RUNTIME.block_on(async {
            let current_version = sgx_def_registration_tool::register_sgx_2_not_fetch(
        ...
        RUNTIME.block_on(async {
            sgx_def_registration_tool::register_sgx_test(false).await;
        });
...
fn initialize_seal_key() -> Vec<u8> {
    sgx_def_registration_tool::ONLINESK
        .read()
        .unwrap()
        .as_ref()
        .unwrap()
